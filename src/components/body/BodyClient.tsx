...
import {
  FloatingScanParticles,
  OrbitalAvatarHalo,
} from "@/components/fx/FutureBodyFx";
...
export function BodyClient() {
  ...
  const overallScore = Math.round(
    systems.reduce((sum, item) => sum + item.score, 0) /
  ...
  return (
    <main className="w-full overflow-x-hidden px-4 py-4 md:px-6">
      <section className="mx-auto grid w-full max-w-[1760px] gap-4 xl:grid-cols-[320px_minmax(0,1fr)_440px]">
        <HUDCard className="xl:col-span-3">
          <div className="relative flex min-h-[96px] items-center justify-between overflow-hidden rounded-[inherit] px-6 py-5">
        ...
        <div className="space-y-4">
          <HUDCard className="p-5">
            ...
            <div className="mt-3 text-center">
              ...
              <div
                className={`mt-1 text-xs font-bold uppercase tracking-[0.18em] ${getStatusClass(
                  overallScore
                )}`}
              >
        ...
        <InsightCard
          ...
          body={`Your strongest current body system is ${
            highest?.name || "metabolic"
          }, with a score of ${highest?.score || overallScore}/100.`}
        />
        ...
        <InsightCard
          ...
          body={`The model sees the most room for improvement in ${
            lowest?.name || "neurological"
          }. This is a lifestyle simulation signal, not a diagnosis.`}
        />
...
function BodyScanPanel({
  ...
  return (
    <HUDCard className="relative min-h-[720px] overflow-hidden">
      ...
      >
        ...
        />
        <path
          d="M80 130 H230 L295 210"
          stroke="#22d3ee"
          strokeOpacity=".2"
        />
        <path
          d="M820 130 H670 L600 210"
          stroke="#22d3ee"
          strokeOpacity=".2"
        />
        <path
          d="M90 600 H245 L320 525"
          stroke="#22d3ee"
          strokeOpacity=".18"
        />
        <path
          d="M810 600 H650 L580 525"
          stroke="#22d3ee"
          strokeOpacity=".18"
        />
      </svg>
      ...
      <FloatingSystem
        ...
        value={byId.cardiovascular?.score || 0}
      />
      <FloatingSystem
        className="right-[8%] top-[22%]"
        label="Neurological"
        value={byId.neurological?.score || 0}
      />
      <FloatingSystem
        className="left-[7%] top-[52%]"
        label="Respiratory"
        value={byId.respiratory?.score || 0}
      />
      <FloatingSystem
        className="right-[8%] top-[50%]"
        label="Digestive"
        value={byId.digestive?.score || 0}
      />
      <FloatingSystem
        className="left-[13%] bottom-[18%]"
        label="Immune"
        value={byId.immune?.score || 0}
      />
      <FloatingSystem
        className="right-[10%] bottom-[18%]"
        label="Metabolic"
        value={byId.metabolic?.score || 0}
      />

      <div className="absolute inset-x-8 bottom-8 top-16 z-10 flex items-center justify-center">
        {avatarUrl ? (
          <img
            src={avatarUrl}
            alt="FutureBody avatar"
            className="max-h-full max-w-[58%] rounded-[28px] object-contain drop-shadow-[0_0_55px_rgba(34,211,238,.45)]"
          />
        ) : (
          <Image
            src={assets.body}
...
function FloatingSystem({
  ...
  return (
    <div
      className={`absolute z-30 hidden rounded-xl border border-cyan-300/15 bg-slate-950/65 px-4 py-3 backdrop-blur xl:block ${className}`}
    >
...
function SystemDetailCard({
  ...
  return (
    <HUDCard className="min-h-[132px] p-4">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0">
              <h3 className="break-words text-sm font-bold uppercase tracking-[0.24em] text-white">
                {system.name}
              </h3>
              <div
                className={`mt-1 text-[10px] font-bold uppercase tracking-[0.14em] ${getStatusClass(
                  system.score
                )}`}
              >
            ...
            <div className="shrink-0 text-3xl font-light text-white">
              {system.score}
          ...
          <p className="mt-2 text-sm leading-5 text-slate-400">
            {system.description ||
              fallbackDescriptions[system.id] ||
              "Body system readiness estimate."}
          </p>

          <p className="mt-1 text-xs leading-5 text-slate-500">
            Based on sleep {input.sleep}h, stress {input.stress}/100, steps{" "}
            {input.steps.toLocaleString()} and diet quality {input.diet}/100.
          </p>
  ...
  return (
    <HUDCard className="relative overflow-hidden p-6">
      ...
      <div className="relative z-10 flex flex-col items-center text-center">
        ...
        <div className="mt-3 text-xs font-bold uppercase tracking-[0.18em] text-cyan-300">
          Neural Insight
        </div>
